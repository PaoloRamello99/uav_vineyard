...
# Configura qui il percorso del tuo file YAML
YAML_FILE_PATH = "/workspaces/uav_vineyard/src/automatic_uav_mppi/worlds/config/vineyard_params.yaml"

# Carica la configurazione dei parametri del vigneto dal file YAML
try:
    with open(YAML_FILE_PATH, 'r') as file:
        yaml_data = yaml.safe_load(file)
        # Estrae i parametri dalla struttura ROS 2
        params = yaml_data["vineyard_offboard"]["ros__parameters"]
except FileNotFoundError:
    print(f"Errore: Il file {YAML_FILE_PATH} non è stato trovato.")
    sys.exit(1)
except KeyError as e:
    print(f"Errore: Chiave mancante nel file YAML: {e}")
    sys.exit(1)
